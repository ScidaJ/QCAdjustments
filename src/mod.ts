...
class QuestConditionAdjuster implements IPostDBLoadMod {
    ...
    public postDBLoad(container: DependencyContainer): void {
        ...
        // Config init
        const multipliers = {
            [ConditionType.Counter]: (CONFIG.task_multipliers.counter > 0) ? CONFIG.task_multipliers.counter : this.DEFAULT_TASK_MULTIPLIER,
            [ConditionType.Find]: (CONFIG.task_multipliers.find_handover > 0) ? CONFIG.task_multipliers.find_handover : this.DEFAULT_TASK_MULTIPLIER,
            [ConditionType.Handover]: (CONFIG.task_multipliers.find_handover > 0) ? CONFIG.task_multipliers.find_handover : this.DEFAULT_TASK_MULTIPLIER,
            [ConditionType.LeaveAt]: (CONFIG.task_multipliers.leave_at > 0) ? CONFIG.task_multipliers.leave_at : this.DEFAULT_TASK_MULTIPLIER,
            [ConditionType.Sell]: (CONFIG.task_multipliers.sell > 0) ? CONFIG.task_multipliers.sell : this.DEFAULT_TASK_MULTIPLIER
        }
        ...
        if (CONFIG.gunsmith_kills.enabled) {
            ...
            for (const questId in GSConditionIds) {
                ...
                for (const [index, condition] of quest.conditions.AvailableForFinish.entries()) {
                    let weapon = (typeof condition.target === 'string') ? condition.target : condition.target[0];
                    newConditions.push(CreateGunsmithCondition(GSConditionIds[quest._id][index], weapon, gunsmithKillCount));
                }
                if (replaceTask)
                    newConditions = [...quest.conditions.AvailableForFinish, ...newConditions];
    ...
    private adjustQuestCondition(condition: IQuestCondition, multipliers: { [key in ConditionType]?: number }): void {
